...
description: 原本在 Inertia v2.0 的時候，應該就要一起推出的 Infinite Scroll (無限滾動) 功能，拖了一年之後，終於在 Inertia v2.2 正式推出了！無限滾動通常常見在聊天室訊息、社交媒體動態、照片牆等場景，可以讓使用者無限滑動載入資料，而不需要點擊分頁按鈕。而且我滿喜歡這個 API 的設計，使用起來還滿簡單的說~
