...
如果有使用 Bash，需要增加以下指令到 `~/.bashrc` 檔案中：

```bash
# GPG Agent setup for Git commit signing
export GPG_TTY=$(tty)
...
將 GPG Agent 的快取時間延長到 12 小時，可以在一天中只需輸入一次密碼：

```apache title="C:\Users\[USER]\.gnupg\gpg-agent.conf"
# Cache passphrase for 43200 seconds (12 hours)
default-cache-ttl 43200

# Keep passphrase at most 86400 seconds (24 hours)
max-cache-ttl 86400
```

## WSL 切換成 Windows 原生 GUI 密碼輸入介面

WSL 預設會使用文字介面的密碼輸入框，這裡可以改成 Windows 原生 GUI 密碼輸入框：

```apache title="~/.gnupg/gpg-agent.conf"
