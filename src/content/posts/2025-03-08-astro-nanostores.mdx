---
title: Astro 筆記 (3)：在 Astro 中用 nanostores 共享全局變數
description: 描述...
# image: /images/2025-03-08-astro-nanostores.jpg
# showImage: false
pubDate: 2025-03-08
category: coding
tags: ["Astro"]
...
這篇文章記錄了一下我第一次用 nanostores 做的範例，是訊息提示元件的狀態管理，可以呼叫顯示訊息提示的函數，並在
 2 秒後自動消失。
...
首先先定義一個 `messageStateStore` 在 `src/stores/message.ts` 中，`messageStateStore` 保存了顯示訊息類型和訊息內容，然後寫一個 `showMessage()` 函數來設定呼叫時要顯示的類型和內容，並在 2 秒後會自己消失：
