...
終端機建議使用 Git Bash \(安裝 Git 即會自動安裝Git Bash\)。在 Git Bash 中無法使用 `Ctrl` `C/V` 複製貼上，需要用 `Ctrl` `Insert` 複製、`Shift` `Insert` 貼上。但如果使用 Windows Terminal 的話就可以直接使用 `Ctrl` `C/V` 複製貼上。
...
[安裝phpMyAdmin](/posts/install-phpmyadmin)
...
預設的資料夾為 `C:\Users\[USER]\code`，若 `code` 資料夾不存在須創建。
...
[Homestead 安装问题汇总](https://learnku.com/laravel/t/4796/ssh-auth-method-private-key-solution-summary-when-homestead-starts)
...
* `C:\Users\[USER]\Homestead\.vagrant`
