...
從本地 dev server 的 `/og/<slug>` 預覽頁擷取圖片，存成 `public/images/<slug>.jpg` 供 production 使用。

## Step 1 — 確認參數

使用者通常只給 slug 或網址，其餘用預設值推導，然後把三項一起列出讓使用者確認：

- **URL**：`http://localhost:4321/og/<slug>`
- **尺寸**：`1200x630`（OG 標準尺寸，除非使用者指定）
- **輸出**：`public/images/<slug>.jpg`（絕對路徑）

`public/images/` 會被 commit，檔名寫錯會覆蓋到別篇，所以一定要先確認。
...
1. `new_page` — 開 URL
2. `resize_page` — 套用尺寸（template 排版依 viewport，少這步會跟 production 對不上）
3. `take_screenshot` — `format: 'jpeg'`、`filePath` 用絕對路徑、`fullPage: false`
4. `close_page` — 關掉剛開的分頁，避免 session 累積殘留分頁
