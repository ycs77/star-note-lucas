<div align="center">

# 星星的筆記．Lucas

![星星的筆記．Lucas](screenshot.png)

</div>

部落格 [星星的筆記．Lucas](https://star-note-lucas.me/) 的原始碼。

技術棧使用 [Astro](https://astro.build/)、[Tailwind CSS](https://tailwindcss.com/)、[Vue.js](https://vuejs.org/)，部署在 [Vercel](https://vercel.com/) 平台。

## 功能

- Markdown 文章
- 文章分類
- 文章標籤
- Open Graph 圖片
- RSS 訂閱

## 開發

```bash
pnpm install
pnpm dev
```

## LICENSE

原始碼 (不包含文章及圖片) 採用 [MIT LICENSE](LICENSE) 許可協議。

<a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/"><img alt="創用CC授權條款" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png" /></a><br />本部落格所有文章除特別聲明外，均採用 <a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/">創用CC 姓名標示-非商業性-相同方式分享 4.0 國際授權條款 (CC BY-NC-SA 4.0)</a> 許可協議。
